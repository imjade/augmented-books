...
public class ARLayout extends View implements /*LocationListener,*/ SensorEventListener
{
	...
	{
		Log.e("ARLayout","***** location changed *****");
		if(curLocation == null)
		{
			Log.e("ARLayout","***** location changed and location null *****");
			curLocation = location;
			
		//	ARSphericalView.deviceLocation = location;
			ARButton.deviceLocation = location;
				...
				curLocation.getLongitude() == location.getLongitude())
		{
			...
			Log.e("ARLayout","***** location changed and location is same as old one");	
		}
		else{
			locationChanged = true;
		}
	...
	public void onSensorChanged(SensorEvent evt)
	{
		...
		if(evt.sensor.getType() == Sensor.TYPE_ORIENTATION)
		{
		    ...
		    if(Main.curLocation != null && first){
		    	updateLayouts(localDirection, (float)inclination, Main.curLocation);
		    	first = false;
		    }
		    else
		...
		if(direction < 0)
		 ...
		 if(Main.curLocation != null && first2 ){
		    	updateLayouts(localDirection, (float)inclination, Main.curLocation);
		    	first2 = false;
		 }
		    else
	...
	public void addARView(ARButton view)
	{
		arViews.add(view);
	}
	...
	public void removeARView(ARButton view)
	{
		arViews.remove(view);
		Log.e("ARLayout", "***** REMOVED VIEW *****");
	}
	...
	public void clearARViews()
	{
		...
		Log.e("ARLayout", "***** REMOVED VIEWs *****");
		/*for(int i=0; i<Main.buttonPlaces.size(); i++){
			Main.buttonPlaces.get(i).setVisibility(View.GONE);
		}*/
		for(int i=0; i<Main.linearLayoutPlaces.size(); i++){
			Main.linearLayoutPlaces.get(i).setVisibility(View.GONE);
		}
	}
...
//		}
	...
	public void onDraw(Canvas c)
	{
		...
		Enumeration<ARButton> e = arViews.elements();
		if(debug)
		{
			Paint p = new Paint();
	...
	//				}
				...
				{
					Log.e("ARLayout", "catch in ARLayout");
